...
**Q: 上传 Windows（CRLF 换行）的文件能正常多人协同吗？**  
A: 可以。服务端会统一把文本换行规范化为 LF 存储与处理，使前后端字符偏移一致，避免"一人编辑、他人显示错位"的问题。

**Q: 运行 `mvn test` 会影响 `data/` 里的文档吗？**  
A: 不会。测试数据被隔离到 `target/test-data/`，运行测试不会触碰真实的 `data/` 目录。
